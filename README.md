...
| Python 3.13 | Resolve 21's `fusionscript.dll` expects Python 3.13. The repo includes `.python-version` for `uv`. |
...
```powershell
uv sync --python 3.13.14 --extra transcription
```

This creates `.venv\` and installs everything: `mcp`, `faster-whisper`, `pywin32`, `Pillow`, `onnxruntime`, and all transitive deps.

> **Note:** `onnxruntime` is pinned to the pipeline's validated range in `pyproject.toml`. This range includes Python 3.13 wheels.
...
```powershell
uv sync --python 3.13.14 --extra transcription --extra cuda-win
```
...
The Whisper transcripts (`transcripts/*.json`) and orchestrator LLM packet
...
human-in-the-loop review rounds).
...
transparent to all read/write operations. New transcripts and LLM
packets generated by the orchestrator land directly in `_data/` and can
...
> **Python version:** The venv must use Python 3.13 (matches the `fusionscript.dll` Resolve 21 ships with). If import fails after a Resolve upgrade, recreate the venv with `uv sync --python 3.13.14 --extra transcription --extra cuda-win`.

### End-to-end orchestrator workflow

The canonical full edit pass is the Python orchestrator GUI documented in
`docs/orchestrator_gui.md`. Individual helper scripts are implementation
details for the orchestrator or narrow repair tasks; do not chain them together
as a replacement full-edit workflow.

The orchestrator is intentionally stricter than individual helper scripts:
...
Low-level helper retained for targeted repair work. Full edit generation should
use the orchestrator GUI.
...
For full workflows, battle detection should run as an orchestrator LLM step so
the prompt, output artifact, and JSON validation stay in the GUI-managed
status/cache flow.
...
Low-level helper retained for targeted repair work. Full edit generation should
use the orchestrator GUI. The helper detects the end of each trainer battle from
a prompt packet plus validated JSON result, then places green "Battle End"
markers on the timeline.

```cmd
rem Full helper run: extract frames, write packet, wait for JSON, place markers
...
For full workflows, run this through the orchestrator LLM/status flow so the
prompt, output, and validation remain visible in the GUI.
...
```cmd
rem Full helper run: dense extract -> JSON result -> replace markers
...
**How it works:** Loads the initial estimates from `battle-ends-<stem>.out.md`, extracts ~41 frames at 0.25s steps in a ±5s window around each estimate (~246 frames for 6 battles), writes a refinement packet, and consumes the validated JSON result. In full workflows, this belongs in the orchestrator LLM/status flow. Once the output JSON is available, the script clears existing green markers and places refined ones.
...
Locates the first V1 clip whose first frame shows the carousel style overlay. Extracts first-frame + previous-clip's-last-frame for up to 30 candidates after the last green marker, then uses the orchestrator visual LLM/status flow to identify the start. Places a yellow `Member Carousel Start` marker.
...
The orchestrator LLM step reads `transcripts/battles.json` plus the surrounding transcript context (±30s per battle) and classifies each as `rival`, `gym`, or `other`. Cached to `transcripts/battle-types.json`.
