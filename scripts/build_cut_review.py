...
<script>
var MANUAL_GROUPS=__MANUAL_GROUPS__, AUTO_GROUPS=__AUTO_GROUPS__, STRUCTURAL_GROUPS=__STRUCTURAL_GROUPS__, INITIAL=__INITIAL__;
var dec={}, autoDec={}, structDec={}, CUTS={}, RESTORES={}, STRUCTURAL_RESTORES={}, boxDrag=null;
...
document.querySelectorAll('.card').forEach(function(card){var mode=card.dataset.mode,g=card.dataset.g,info=(activeGroups(mode)[g]||{dur:0,active:[]});var audio=card.querySelector('audio'),ph=card.querySelector('.playhead'),badge=card.querySelector('.nowpink'),ww=card.querySelector('.wavewrap'),boxes=boxStore(mode);var initialBoxes=(mode==='auto'?INITIAL.restores:(mode==='structural'?INITIAL.structural_restores:INITIAL.cuts))||{};boxes[g]=initialBoxes[g]?initialBoxes[g].map(function(c){return c.slice();}):[];var prevMode=false,skips=[];
  ...
  function loop(){if(audio.paused)return;if(prevMode){for(var i=0;i<skips.length;i++){if(audio.currentTime>=skips[i][0]&&audio.currentTime<skips[i][1]-0.005){audio.currentTime=skips[i][1];break;}}}var f=audio.currentTime/dur();if(f>1)f=1;ph.style.left=(f*100)+'%';var inA=info.active.some(function(r){return audio.currentTime>=r[0]&&audio.currentTime<=r[1];});ph.classList.toggle('red',inA);badge.classList.toggle('show',inA);requestAnimationFrame(loop);}
  audio.addEventListener('play',function(){ph.classList.add('vis');requestAnimationFrame(loop);});audio.addEventListener('pause',function(){prevMode=false;});audio.addEventListener('ended',function(){prevMode=false;badge.classList.remove('show');});audio.addEventListener('loadedmetadata',render);
...
document.getElementById('save').onclick=function(){var cuts={},restores={},structuralRestores={};for(var g in CUTS)if(CUTS[g].length)cuts[g]=CUTS[g];for(var h in RESTORES)if(RESTORES[h].length)restores[h]=RESTORES[h];for(var sg in STRUCTURAL_RESTORES)if(STRUCTURAL_RESTORES[sg].length)structuralRestores[sg]=STRUCTURAL_RESTORES[sg];var j=JSON.stringify({pink:dec,cuts:cuts,auto:autoDec,restores:restores,structural:structDec,structural_restores:structuralRestores},null,1);document.getElementById('out').value=j;var b=new Blob([j],{type:'application/json'});var a=document.createElement('a');a.href=URL.createObjectURL(b);a.download='pink_decisions.json';a.click();};
</script></body></html>"""
