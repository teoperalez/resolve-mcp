...
**10. Source-frame audio duplication** (DO NOT FLAG — leave the duplicate). When a clip is flagged `!!SRC_OVERLAP_PREV=Xs`, X seconds of source frames play TWICE on the rendered timeline due to a mechanical bug in upstream battle-gap insertion. **Do not flag this as a cut.** The downstream `apply_cuts_to_fcpxml.py` operates on source-frame ranges, not on individual clip-position instances — when you flag a cut at the overlap range, it removes those source frames from BOTH copies (this clip AND the previous clip whose tail also covers that range). Result: the duplicated WORDS get cut entirely, not just the duplicate copy. Confirmed bug 2026-05-15: cutting "rival number two" overlap removed those words from both clips, leaving "rival for now though" in the rendered audio. The right fix for SRC_OVERLAP is in the upstream tool, not the cut analyzer. Tolerate the brief duplication — it's mildly awkward but does not lose narrative content.
...
- **Atomic numbered references.** Phrases that name a numbered entity travel as a UNIT. Either the whole phrase is in or the whole phrase is out — never split. A cut whose boundary lands between the entity name and its number is wrong by definition. Examples: *"Rival 2"*, *"rival number two"*, *"rival 2 attempt number one"*, *"the second gym leader"*, *"attempt number one"*, *"reset 29"*, *"my second Pokemon"*, *"the third E4 member"*. Same rule for digits ("2"), words ("two"), and ordinals ("second"). When evaluating any cut, check whether either edge splits a `<noun> <number-or-ordinal>` sequence. If yes, extend or shrink the cut so the unit stays together. Real regression caught 2026-05-15: cutting source 342.92-343.62 split *"rival number two"*, leaving the listener with *"if she can get through rival for now though..."* — broken sentence, lost reference. Bug originated in SRC_OVERLAP_PREV cut (now forbidden), but the atomic rule applies to ALL cuts.
...
5. **Does either edge of the cut split a `<noun> <number/ordinal>` reference?**
   - Look at the words immediately before `start_sec` and immediately after `end_sec`. If the cut start lands BETWEEN a noun and its number/ordinal (e.g. "rival" → cut → "number two"), or the cut end lands likewise (e.g. "rival number" → cut → "two"), the cut is splitting an atomic numbered reference. **Fix the boundary**: either extend the cut to swallow the whole phrase, or shrink it so the phrase stays intact on one side. Numbered references include: rivals (Rival 2 / rival number three), gyms (the second gym leader), attempts (attempt number one), resets (reset 29), Pokemon position (my second Pokemon), E4 members (the third E4 member). Whisper renders numbers as digits ("2"), words ("two"), or ordinals ("second") — all three forms count.
...
## Borderline cases — flag for audio verification, do not guess

If you are genuinely undecided whether a moment is (a) emphatic restatement Teo would intentionally leave in vs (b) a false-start/duplicate that should be cut — and you cannot resolve it from the transcript alone — DO NOT GUESS in either direction. Add the cut entry with `confidence: medium` AND include the literal word `borderline` in the `reason` string and a brief description of WHY you are uncertain (e.g. "borderline — could be emphatic restatement of 'I've gotten it down to eight' OR could be aborted first attempt; cannot tell from transcript alone").

The transcript driving this analysis is `turbo`-quality and has known failure modes: it merges duplicate words ("barriers, so these berries" → "berries"), drops aborted partials ("eight" → "h"), and won't help you decide intent in these cases. The main-thread reviewer has a documented audio-verification procedure (`reference_audio_verify_borderline_cuts.md` in user memory): re-extract the disputed window with ffmpeg, re-transcribe with `large-v3` + `word_timestamps=True` + `vad_filter=False`, and check 50ms-bin waveform energy for silence boundaries. Two minutes per disputed clip resolves the question definitively.

By marking the entry `borderline`, you defer the decision to audio analysis instead of guessing. The reviewer will either confirm the cut (and remove the borderline tag) or revert it. **Do not silently downgrade or skip a moment because you're unsure** — flagging it `borderline` is the correct disposition.
