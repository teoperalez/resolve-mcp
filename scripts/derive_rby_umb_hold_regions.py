...
def derive_regions(
    ...
    *,
    ...
    # Post-battle data/tier cards: state 2 opens, state 1 closes.
    n = 1
    for ev in events:
        ...
        if final_start_s is not None and end_s >= final_start_s:
            continue
        r = make_region(
            label=f"post_battle_data_card_{n:02d}",
            kind="post_battle_data_card",
            start_sec=start_s,
            end_sec=end_s,
            source_offset=source_offset,
            fps=fps,
            reason="post-battle score/time/split card should stay visually continuous",
            source_dur=source_dur,
